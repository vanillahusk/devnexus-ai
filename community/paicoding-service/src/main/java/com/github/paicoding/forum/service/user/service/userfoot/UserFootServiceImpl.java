...
public class UserFootServiceImpl implements UserFootService {
    ...
    public UserFootServiceImpl(UserFootDao userFootDao, MqOutboxService mqOutboxService) {
    ...
    public UserFootDO saveOrUpdateUserFoot(DocumentTypeEnum documentType, Long documentId, Long authorId, Long userId, OperateTypeEnum operateTypeEnum) {
        return saveOrUpdateUserFootWithChange(documentType, documentId, authorId, userId, operateTypeEnum).foot();
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public UserFootUpdateResult saveOrUpdateUserFootWithChange(DocumentTypeEnum documentType, Long documentId,
                                                               Long authorId, Long userId,
        ...
        if (readUserFootDO == null) {
            ...
            changed = setUserFootStat(readUserFootDO, operateTypeEnum);
        ...
        return new UserFootUpdateResult(readUserFootDO, changed);
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public UserFootUpdateResult saveOrUpdateUserFootWithOutbox(DocumentTypeEnum documentType, Long documentId,
                                                               Long authorId, Long userId,
                                                               OperateTypeEnum operateTypeEnum, String eventId,
                                                               Long operationVersion) {
        if (operationVersion == null || operationVersion <= 0) {
            throw new IllegalArgumentException("favor operationVersion must be positive");
        }
        UserFootDO foot = userFootDao.getByDocumentAndUserIdForUpdate(
                documentId, documentType.getCode(), userId);
        boolean changed = false;
        if (foot == null) {
            foot = new UserFootDO();
            foot.setUserId(userId);
            foot.setDocumentId(documentId);
            foot.setDocumentType(documentType.getCode());
            foot.setDocumentUserId(authorId);
            changed = setUserFootStat(foot, operateTypeEnum);
            foot.setFavorVersion(operationVersion);
            userFootDao.save(foot);
        } else if (foot.getFavorVersion() == null || operationVersion > foot.getFavorVersion()) {
            changed = setUserFootStat(foot, operateTypeEnum);
            foot.setFavorVersion(operationVersion);
            foot.setUpdateTime(new Date());
            userFootDao.updateById(foot);
        }
        UserFootUpdateResult result = new UserFootUpdateResult(foot, changed);
        if (result.changed()
                && (operateTypeEnum == OperateTypeEnum.PRAISE || operateTypeEnum == OperateTypeEnum.CANCEL_PRAISE)) {
            mqOutboxService.saveFavorNotify(eventId, OperateTypeEnum.getNotifyType(operateTypeEnum), result.foot());
        }
        return result;
    }
