...
public class CommentDao extends ServiceImpl<CommentMapper, CommentDO> {
    ...
    public List<CommentDO> listTopCommentList(Long articleId, PageParam pageParam) {
        return lambdaQuery()
                ...
                .eq(CommentDO::getAuditStatus, CommentAuditStatusEnum.APPROVED.getCode())
    ...
    public CommentDO getHotComment(Long articleId) {
        ...
        return getApprovedById(Long.parseLong(String.valueOf(map.get("top_comment_id"))));
    }

    public CommentDO getApprovedById(Long commentId) {
        return lambdaQuery()
    ...
    public CommentDO getBySourceEventId(String sourceEventId) {
        if (sourceEventId == null || sourceEventId.isBlank()) {
            return null;
        }
        return lambdaQuery().eq(CommentDO::getSourceEventId, sourceEventId).one();
    }

    public boolean transitionAuditStatus(Long commentId, CommentAuditStatusEnum target) {
        return lambdaUpdate()
                .eq(CommentDO::getId, commentId)
                .eq(CommentDO::getAuditStatus, CommentAuditStatusEnum.PENDING.getCode())
                .eq(CommentDO::getDeleted, YesOrNoEnum.NO.getCode())
                .set(CommentDO::getAuditStatus, target.getCode())
                .update();
    }
