...
public interface UserFootService {
    ...
    /**
     * 保存状态并返回业务状态是否真的发生变化。异步点赞链路据此避免重复请求导致计数和通知重复。
     */
    UserFootUpdateResult saveOrUpdateUserFootWithChange(DocumentTypeEnum documentType, Long documentId,
                                                        Long authorId, Long userId,
                                                        OperateTypeEnum operateTypeEnum);

    /** 在同一数据库事务内更新点赞状态并写入待发送 Outbox 事件。 */
    UserFootUpdateResult saveOrUpdateUserFootWithOutbox(DocumentTypeEnum documentType, Long documentId,
                                                        Long authorId, Long userId,
                                                        OperateTypeEnum operateTypeEnum, String eventId,
                                                        Long operationVersion);

    record UserFootUpdateResult(UserFootDO foot, boolean changed) {
    }
