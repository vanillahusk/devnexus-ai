...
public class CommentReadServiceImpl implements CommentReadService {
    ...
    public CommentDO queryComment(Long commentId) {
        return commentDao.getApprovedById(commentId);
    }

    @Override
    public List<TopCommentDTO> getArticleComments(Long articleId, PageParam page) {
        CommentHotspotGovernanceService.CommentForestSnapshot snapshot = commentHotspotGovernanceService.loadForestSnapshot(
                articleId,
                page.getPageNum(),
                page.getPageSize(),
                () -> {
                    List<CommentDO> topComments = commentDao.listTopCommentList(articleId, page);
                    if (CollectionUtils.isEmpty(topComments)) {
                        return CommentHotspotGovernanceService.CommentForestSnapshot.of(Collections.emptyList(), Collections.emptyList());
                    }
                    List<CommentDO> subComments = commentDao.listSubCommentIdMappers(articleId,
                            topComments.stream().map(CommentDO::getId).collect(Collectors.toList()));
                    return CommentHotspotGovernanceService.CommentForestSnapshot.of(topComments, subComments);
                }
        );

        // 1.查询一级评论
        List<CommentDO> comments = snapshot.getTopComments();
        ...
        // 2.查询非一级评论
        List<CommentDO> subComments = snapshot.getSubComments();
        ...
        comments.forEach(comment -> {
            ...
            fillTopCommentInfo(articleId, dto);
            result.add(dto);
        });
     ...
     * @param comment
     */
    private void fillTopCommentInfo(Long articleId, TopCommentDTO comment) {
        fillCommentInfo(comment);
        comment.getChildComments().forEach(this::fillCommentInfo);
        Collections.sort(comment.getChildComments());
        comment.setCommentCount(commentHotspotGovernanceService.queryReplyCount(articleId,
    ...
    public TopCommentDTO queryHotComment(Long articleId) {
        ...
        Long hotCommentId = commentHotspotGovernanceService.queryHotTopCommentId(articleId);
        if (hotCommentId != null) {
            comment = commentDao.getApprovedById(hotCommentId);
            if (comment != null && (!Objects.equals(comment.getArticleId(), articleId) || !Objects.equals(comment.getTopCommentId(), 0L))) {
                comment = null;
            }
        }
        if (comment == null) {
            comment = commentDao.getHotComment(articleId);
        }
        if (comment == null) {
            return null;
        }
        ...
        // 填充评论信息
        fillTopCommentInfo(articleId, result);
        return result;
    }
