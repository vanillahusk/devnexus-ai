...
public class ArticleDao extends ServiceImpl<ArticleMapper, ArticleDO> {
    ...
    /**
     * 在当前数据库事务内原子递增并读取知识索引版本。
     */
    public long nextKnowledgeVersion(Long articleId) {
        if (articleMapper.incrementKnowledgeVersion(articleId) != 1) {
            throw new IllegalStateException("article knowledge version increment failed, articleId=" + articleId);
        }
        ArticleDO article = articleMapper.selectById(articleId);
        if (article == null || article.getKnowledgeVersion() == null) {
            throw new IllegalStateException("article knowledge version not found, articleId=" + articleId);
        }
        return article.getKnowledgeVersion();
    }
     ...
     * 查询供知识检索使用的当前公开文章快照。
     *
     * <p>ONLINE 与未删除条件在同一条文章查询中完成，避免公开详情接口对 OFFLINE/REVIEW
     * 内容的特殊可见规则，也不会产生阅读计数或用户足迹副作用。</p>
     */
    public AiKnowledgeArticleSnapshotDTO queryOnlineKnowledgeSnapshot(Long articleId) {
        ...
        ArticleDetailDO detail = articleDetailMapper.selectOne(Wrappers.<ArticleDetailDO>lambdaQuery()
        ...
        return AiKnowledgeArticleSnapshotDTO.builder()
                .articleId(article.getId())
                .articleVersion(article.getKnowledgeVersion() == null
                        ? Math.max(1L, detail.getVersion() == null ? 1L : detail.getVersion())
                        : article.getKnowledgeVersion())
                .title(article.getTitle())
                .summary(article.getSummary())
                .content(detail.getContent())
                .categoryId(article.getCategoryId())
                .updatedAt(article.getUpdateTime() == null ? null : article.getUpdateTime().getTime())
                .build();
    }
