...
public class ReqRecordFilter implements Filter {
    ...
    private HttpServletRequest initReqInfo(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            // 优先沿用 Gateway 生成的安全关联 ID；非法或缺失时在本服务重新生成。
            String incomingTraceId = request.getHeader(TRACE_ID_HEADER);
            if (StringUtils.isNotBlank(incomingTraceId) && SAFE_TRACE_ID.matcher(incomingTraceId).matches()) {
                MdcUtil.setTraceId(incomingTraceId);
            } else {
                MdcUtil.addTraceId();
            }
            stopWatch.stop();
...
//            stopWatch.stop();
            ...
            response.setHeader(TRACE_ID_HEADER, Optional.ofNullable(MdcUtil.getTraceId()).orElse(""));
            stopWatch.stop();
        } catch (Exception e) {
