...
import com.github.paicoding.forum.service.article.cache.HotArticleBasicCacheService;
...
public class ArticleRestController {
    ...
    @Autowired
    private HotArticleBasicCacheService hotArticleBasicCacheService;

    @Autowired
    private FavorAsyncWriteService favorAsyncWriteService;
...
//        list.forEach(c -> c.setSelected(c.getCategoryId().equals(categoryId)));
    ...
    public ResVo<Boolean> favor(@RequestParam(name = "articleId") Long articleId,
                                @RequestParam(name = "type") Integer type) throws IOException {
        ...
        if (!favorAsyncWriteService.allowFavorRequest(articleId, userId)) {
            return ResVo.fail(StatusEnum.ILLEGAL_ARGUMENTS_MIXED, "请求过于频繁");
        }

        // 要求文章必须存在
        ArticleDO article = hotArticleBasicCacheService.queryBasicArticle(articleId);
        if (article == null) {
        ...
        event.setAuthorId(article.getUserId());
        event.setUserId(userId);
        event.setOperateType(operate.getCode());
        favorAsyncWriteService.enqueue(event);
