...
public class TestController {
    ...
    public ResVo<String> email(EmailReqVo req) {
        ...
        boolean ans = sendMail(req.getTitle(), req.getTo(), req.getContent());
        log.info("测试邮件发送，计数：{}，发送内容：{}", cnt.addAndGet(1), req);
        return ResVo.ok(String.valueOf(ans));
    }

    /** 隔离静态邮件客户端，便于测试在不动态挂载 Java Agent 的环境中替换副作用。 */
    protected boolean sendMail(String title, String to, String content) {
        return EmailUtil.sendMail(title, to, content);
    }
    ...
    public ResVo<Boolean> favorByMessage(@RequestParam(name = "articleId") Long articleId,
        ...
        // 点赞消息走当前启用的 MQ Provider，其它走 Java 内置消息机制
        if ((notifyType.equals(NotifyTypeEnum.PRAISE) || notifyType.equals(NotifyTypeEnum.CANCEL_PRAISE)) && messageQueueService.enabled()) {
            messageQueueService.publish(new MessageQueueEvent<>(notifyType, foot), CommonConstants.ROCKETMQ_TAG_NOTIFY);
        } else {
