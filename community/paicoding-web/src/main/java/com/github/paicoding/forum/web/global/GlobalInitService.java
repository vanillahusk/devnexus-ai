...
public class GlobalInitService {
    ...
    @Resource
    private NotifyReadFacade notifyReadFacade;
    ...
    public void initLoginUser(String session, ReqInfoContext.ReqInfo reqInfo) {
        BaseUserInfoDTO user = authFacade.resolveUserBySession(session, null);
        reqInfo.setSession(session);
        if (user != null) {
            reqInfo.setUserId(user.getUserId());
            reqInfo.setUser(user);
            reqInfo.setMsgNum(notifyReadFacade.queryUserNotifyMsgCount(user.getUserId()));
        }
    }
}
