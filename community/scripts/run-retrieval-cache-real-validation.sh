...
TIMEOUT_SECONDS="${RAGENT_RETRIEVAL_CACHE_TIMEOUT_SECONDS:-180}"
...
RAGENT_REAL_REDIS_RETRIEVAL_CACHE=true \
...
timeout --signal=TERM --kill-after=20s "${TIMEOUT_SECONDS}" \
  ...
  -DskipITs \
  -Dtest=TrustedRetrievalCacheRealIntegrationTest \
  -Dsurefire.failIfNoSpecifiedTests=false test

echo "[OK] real Redis retrieval-cache validation passed; isolated keys were removed"
