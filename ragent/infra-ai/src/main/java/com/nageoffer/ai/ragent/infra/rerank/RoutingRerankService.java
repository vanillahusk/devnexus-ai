...
public class RoutingRerankService implements RerankService {
    ...
    public RoutingRerankService(ModelSelector selector, ModelRoutingExecutor executor,
                                ModelCallConcurrencyGovernor concurrencyGovernor, List<RerankClient> clients) {
    ...
    @Override
    @Trace(operationName = "rag.model.rerank")
    public List<RetrievedChunk> rerank(String query, List<RetrievedChunk> candidates, int topN) {
        return concurrencyGovernor.execute(ModelCapability.RERANK, () -> executor.executeWithFallback(
