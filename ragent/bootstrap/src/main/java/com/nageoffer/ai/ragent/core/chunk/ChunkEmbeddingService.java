...
public class ChunkEmbeddingService {
    ...
    private final EmbeddingVectorCache embeddingCache;
    private final ChunkEmbeddingMetrics metrics;

    public ChunkEmbeddingService(ModelSelector modelSelector, List<EmbeddingClient> embeddingClients,
                                 EmbeddingVectorCache embeddingCache, ChunkEmbeddingMetrics metrics) {
    ...
    public void embed(List<VectorChunk> chunks, String embeddingModel) {
        ...
        List<VectorChunk> misses = new ArrayList<>();
        ...
        for (VectorChunk chunk : chunks) {
            ...
            String key = cacheKey(target.id(), chunk);
        ...
        metrics.chunks("preexisting", preexistingCount);
        metrics.chunks("cached", cachedCount);
        if (misses.isEmpty()) return;
        io.micrometer.core.instrument.Timer.Sample sample = metrics.startBatch();
        try {
            List<List<Float>> vectors = embedBatch(misses, target);
            applyEmbeddings(misses, vectors);
            for (int i = 0; i < misses.size(); i++) {
                embeddingCache.put(missKeys.get(i), misses.get(i).getEmbedding());
            }
            metrics.chunks("computed", misses.size());
            metrics.finishBatch(sample, "success");
    ...
    private String cacheKey(String modelId, VectorChunk chunk) {
        Object hash = chunk.getMetadata() == null ? null : chunk.getMetadata().get("contentHash");
        String contentIdentity = hash == null
                ? ChunkIdentityNormalizer.contentHash(chunk.getContent()) : hash.toString();
        return modelId + ':' + contentIdentity;
    }
