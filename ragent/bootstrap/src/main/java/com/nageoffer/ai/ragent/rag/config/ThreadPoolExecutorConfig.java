...
public class ThreadPoolExecutorConfig {
     ...
     * 对话记忆读取线程池。
     *
     * <p>摘要与最近窗口需要并行读取，但不能落入 JVM 公共 ForkJoinPool，避免慢 SQL
     * 或存储抖动占满其他业务共享线程。队列满时直接拒绝，调用方会降级为空记忆。</p>
     */
    @Bean
    public Executor conversationMemoryReadExecutor() {
        ThreadPoolExecutor executor = new ThreadPoolExecutor(
                2,
                4,
                60,
                ...
                ThreadFactoryBuilder.create()
                        .setNamePrefix("conversation_memory_read_executor_")
                        .build(),
