...
CREATE INDEX idx_kv_collection ON t_knowledge_vector (collection_name);
...
COMMENT ON COLUMN t_knowledge_vector.collection_name IS '物理索引Generation名称';
...
CREATE TABLE t_index_generation (
...
COMMENT ON TABLE t_index_generation IS '索引双Generation重建、增量水位与原子别名状态';

CREATE TABLE t_vector_document_identity (
...
CREATE INDEX idx_vector_document_business
    ON t_vector_document_identity (logical_collection, source_type, business_id, business_version);
COMMENT ON TABLE t_vector_document_identity IS '跨Generation文档业务身份，用于更新和下线时清理不同docId的旧Chunk';
